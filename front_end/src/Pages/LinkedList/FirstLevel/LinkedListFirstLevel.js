    ...
    <div>
      <div className="all-div">
        ...
        <div className="other">
          ...
          />
          <p>Enter the list values after deleting the node at 'head{new Array(deleteStep).fill('.next').join('')}'.</p>
          <div className="game-container">
            {nodes.map((node, index) => (
              <div key={node.id} id={node.id} className={`box-style ${index === deleteStep ? "deleted-node" : ""}`}>
                <p className={`${index === 0 ? "head-text single-head" : ""}`}>{index === 0 ? "Head" : ""}</p>
                <p>{node.value}</p>
          ...
          />
          <button onClick={handleGuessSubmit} className="push-buttons-styling">Submit Guess</button>
          {taskCompleted && <div className="result-message">{resultMessage}</div>}
        </div>
      </div>
    </div>
  );
